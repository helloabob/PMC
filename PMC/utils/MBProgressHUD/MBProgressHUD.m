...
- (void)layoutSubviews {
    ...
    // wangbo minus 50 for the navigation controller height.
    indFrame.origin.x = floorf((frame.size.width - indFrame.size.width) / 2) + self.xOffset;
    indFrame.origin.y = floorf((frame.size.height - indFrame.size.height) / 2) + self.yOffset - 50;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect boxRect = CGRectMake(roundf((allRect.size.width - self.width) / 2) + self.xOffset,
                                roundf((allRect.size.height - self.height) / 2) + self.yOffset - 50, self.width, self.height);
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
    ...
    NSLog(@"rotation changeed!%d",orientation);
